...
adtracker.json: {"meta":{spend,impressions,reach,frequency,clicks,link_clicks,ctr,cpc,cpm,lpv,start},
                 "ad_spend":float (account lifetime), "ad_subs":[{email,plan,amount,interval,status,start,canceled,collected}],
                 "generated":str}
Focused on the AD CAMPAIGN only — delivery, the ad-driven customers, and ROI. Pure openpyxl (no recalc)."""
import json, sys
...
ws.append(["Customer","Plan","Started","Status","Canceling","Paid so far"])
...
for r in subs:
    ws.append([r["email"], r.get("plan",""), r.get("start",""), smap.get(r.get("status"), str(r.get("status")).title()),
               r.get("canceled",""), r.get("collected",0)])
last=len(subs)+1
for c in range(1,7):
...
for i,w in zip("ABCDEF",[34,14,12,12,12,12]): ws.column_dimensions[i].width=w
...
def card(anchor,label,val,fmt,color,fill,blue_input=False):
    col=anchor[0]; row=int(anchor[1:]); nx=chr(ord(col)+1)
    db[f"{col}{row}"].value=label; db[f"{col}{row}"].font=Fn(size=9,bold=True,color=GREY)
    v=db[f"{col}{row+1}"]; v.value=val; v.font=Fn(size=18,bold=True,color="0000FF" if blue_input else color); v.number_format=fmt
...
S="'Ad Customers'"; F=f"{S}!F2:F{last}"; D=f"{S}!D2:D{last}"
db["A10"]="RESULTS & ROI  (what the ad earned)"; db["A10"].font=Fn(bold=True,size=12,color=PURPLE)
card("A11","AD CUSTOMERS",f"=COUNTA({S}!A2:A{last})","#,##0",PURPLE,LAV)
card("C11","STILL ACTIVE",f'=COUNTIF({D},"Active")',"#,##0",GREEN,LAV)
card("E11","COLLECTED SO FAR",f"=SUM({F})","$#,##0.00",GREEN,LAV)
card("G11","ROI (collected ÷ spend)",f"=IF(J2=0,0,SUM({F})/J2)","0%",GREEN,LAV)
card("A13","$ BACK PER $1 SPENT",f"=IF(J2=0,0,SUM({F})/J2)","$#,##0.00",GREEN,LAV)
db["C14"]="Note: collected = first payments so far. As these subs renew, ROI climbs."; db["C14"].font=Fn(size=9,italic=True,color=GREY)
db.merge_cells("C13:H13")

# chart: spend vs collected
db["N20"]="Ad spend"; db["O20"]="=J2"; db["N21"]="Collected"; db["O21"]=f"=SUM({F})"
